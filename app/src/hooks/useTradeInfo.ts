...
import UniswapV2Factory from '@uniswap/v2-core/build/UniswapV2Factory.json's
...
const useTradeExecutionCallback = useCallback(async (signer: ethers.Signer, transaction): Promise<void> => {
...
const useTradeInfo = async (
  ...
  const optionEntities = await Protocol.getOptionsUsingMultiCall(chainId, [optionAddress], provider)
